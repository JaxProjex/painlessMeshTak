...
const char* ssid = "esp8266-AP"; // Change this to your desired SSID
const char* password = "";  // Change this to your desired password, leave blank for open wifi access point

SoftwareSerial mySerial(RX_PIN);
WiFiUDP udp;
IPAddress multicastIP(239, 2, 3, 1);    // Multicast group address
...
void setup() {
  ...
  udp.beginMulticast(WiFi.softAPIP(), multicastIP, multicastPort);
}
  ...
  if (error) {
    ...
    Serial.println(error.c_str());
      ...
      if (cotType == "user") {
        ...
        udp.beginPacketMulticast(multicastIP, multicastPort, WiFi.softAPIP());
